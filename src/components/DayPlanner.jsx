...
export default function DayPlanner({ clients }) {
  const [planned, setPlanned] = useState([]) // [{...client, order}]
  ...
  const removeClient = (id) => {
    setPlanned(prev => prev.filter(c => c.id !== id))
  }
  ...
  const moveDown = (idx) => {
    if (idx === planned.length - 1) return
    setPlanned(prev => {
      const arr = [...prev]
  ...
  const openMaps = () => {
    ...
    const addresses = planned.map(c => encodeURIComponent(c.address || c.name)).join('/')
    window.open(`https://www.google.com/maps/dir/${addresses}`, '_blank')
  }

  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 28, display: 'flex', flexDirection: 'column', gap: 0 }}>
      {/* Header */}
            ...
            >
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {c.lastVisit && (
                  <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 1 }}>
                    ost. wizyta: {formatDate(c.lastVisit)}
                  </div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                <button
                  onClick={() => moveUp(idx)}
                  disabled={idx === 0}
                  style={{ width: 22, height: 22, borderRadius: 4, border: '1px solid var(--border)', background: 'var(--surface2)', color: idx === 0 ? 'var(--border)' : 'var(--muted)', fontSize: 11, cursor: idx === 0 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >▲</button>
                <button
                  onClick={() => moveDown(idx)}
                  disabled={idx === planned.length - 1}
                  style={{ width: 22, height: 22, borderRadius: 4, border: '1px solid var(--border)', background: 'var(--surface2)', color: idx === planned.length - 1 ? 'var(--border)' : 'var(--muted)', fontSize: 11, cursor: idx === planned.length - 1 ? 'default' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}
                >▼</button>
              </div>
      ...
      {planned.length > 0 && (
        <div style={{ marginTop: 14, paddingTop: 12, borderTop: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <span style={{ fontSize: 12, fontFamily: 'var(--mono)', color: 'var(--muted)' }}>
            {planned.length} {planned.length === 1 ? 'wizyta' : planned.length < 5 ? 'wizyty' : 'wizyt'} zaplanowane
          </span>
          <button
            onClick={() => setPlanned([])}
            style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--muted)', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}
          >
            wyczyść listę
          </button>
        </div>
