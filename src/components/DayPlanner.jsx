...
export default function DayPlanner({ clients, onClientClick }) {
  ...
  const q = search.trim()
  const filtered = q
    ? safeClients.filter(c => matchSearch(c.name, q) || matchSearch(c.address || '', q))
    : []
  ...
  const moveUp = (idx) => {
    if (idx === 0) return
    setList(prev => { const a = [...prev]; [a[idx-1], a[idx]] = [a[idx], a[idx-1]]; return a })
  }
  const moveDown = (idx) => {
    if (idx === list.length - 1) return
    setList(prev => { const a = [...prev]; [a[idx], a[idx+1]] = [a[idx+1], a[idx]]; return a })
  }
  ...
  return (
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 28, display: 'flex', flexDirection: 'column' }}>
              ...
              : filtered.map(c => {
                ...
                return (
                  <div
                    ...
                    style={{ padding: '10px 16px', cursor: already ? 'default' : 'pointer', display: 'flex', alignItems: 'center', gap: 10, opacity: already ? 0.45 : 1, borderBottom: '1px solid var(--border)' }}
      ...
      {loading && (
        <div style={{ textAlign: 'center', padding: '24px', color: 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 12 }}>Ładowanie planu...</div>
      )}
