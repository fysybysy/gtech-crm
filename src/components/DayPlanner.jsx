...
export default function DayPlanner({ clients }) {
  ...
  // Expose addClient globally so ClientDetail can call it
  useEffect(() => {
    ...
    window.__dayPlannerHas = (id) => {
      return list.some(x => x.id === id)
    }
    return () => {
      delete window.__dayPlannerAdd
      delete window.__dayPlannerHas
    }
  }, [list])
    ...
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 14, padding: 28, display: 'flex', flexDirection: 'column' }}>
      ...
      {list.length === 0 && (
        <div style={{ textAlign: 'center', padding: '32px 16px', color: 'var(--muted)', fontFamily: 'var(--mono)', fontSize: 12, border: '1px dashed var(--border)', borderRadius: 8 }}>
          Wyszukaj klienta lub użyj przycisku „Dodaj do planu dnia" w szczegółach klienta
        </div>
      )}
      ...
      {list.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, maxHeight: 360, overflowY: 'auto' }}>
          {list.map((c, idx) => (
            <div
              key={c.id}
              style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 10, padding: '12px 14px' }}
            >
              {/* Number badge */}
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {c.address && (
                  <div style={{ fontSize: 11, color: 'var(--muted)', fontFamily: 'var(--mono)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', marginTop: 2 }}>{c.address}</div>
                )}
                {c.lastVisit && (
                  <div style={{ fontSize: 10, color: 'var(--muted)', fontFamily: 'var(--mono)', marginTop: 2 }}>ost. wizyta: {formatDate(c.lastVisit)}</div>
                )}
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 }}>
                <button
                  onClick={() => moveUp(idx)}
                  disabled={idx === 0}
                  style={{ width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--surface2)', color: idx === 0 ? 'var(--border)' : 'var(--text)', fontSize: 10, cursor: idx === 0 ? 'default' : 'pointer' }}
                >▲</button>
                <button
                  onClick={() => moveDown(idx)}
                  disabled={idx === list.length - 1}
                  style={{ width: 22, height: 22, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 4, border: '1px solid var(--border)', background: 'var(--surface2)', color: idx === list.length - 1 ? 'var(--border)' : 'var(--text)', fontSize: 10, cursor: idx === list.length - 1 ? 'default' : 'pointer' }}
                >▼</button>
              </div>
