...
function ClientRow({ c, onClick, showDays }) {
  ...
  return (
    <div
      onClick={() => onClick(c)}
      style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg)', cursor: 'pointer', marginBottom: 8 }}
    ...
    >
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        {c.address && <div style={{ fontSize: 11, fontFamily: 'var(--mono)', color: 'var(--muted)', marginTop: 2, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.address}</div>}
        {days !== null && (
    ...
    <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderTop: `3px solid ${accent}`, borderRadius: 12, padding: 20, display: 'flex', flexDirection: 'column' }}>
      ...
      {clients.length === 0 ? (
        ...
        <>
          ...
          {clients.length > 5 && (
            <button onClick={() => setExpanded(e => !e)} style={{ marginTop: 8, background: 'none', border: 'none', color: accent, fontFamily: 'var(--mono)', fontSize: 12, cursor: 'pointer', textAlign: 'left', padding: '4px 0' }}>
...
export default function AlertPanels({ clients, onClientClick }) {
  ...
  const overdue = clients
    .filter(c => {
      ...
      return (now - new Date(c.lastVisit)) / (1000 * 60 * 60 * 24) >= 14
    })
  ...
  return (
    <div className="dash-grid" style={{ marginTop: 20 }}>
      ...
      <Panel
        ...
        icon="⭐"
        accent="var(--accent2)"
        clients={clients.filter(c => c.stage === 'Nowy').sort((a, b) => (b.visitCount || 0) - (a.visitCount || 0))}
        emptyMsg="Brak klientów w etapie Nowy"
        onClientClick={onClientClick}
        extra="klienci w etapie Nowy"
        showDays={false}
      />
    </div>
  )
