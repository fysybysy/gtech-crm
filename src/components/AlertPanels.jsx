...
function Panel({ title, icon, accent, clients, emptyMsg, onClientClick, extra }) {
      ...
      {clients.length === 0 ? (
        ...
        <>
          <div style={{ flex: 1, overflowY: expanded ? 'auto' : 'visible', maxHeight: expanded ? 400 : 'none' }}>
            {visible.map(c => <ClientRow key={c.id} c={c} onClick={onClientClick} />)}
          </div>
          {clients.length > 5 && (
            <button
...
export default function AlertPanels({ clients, onClientClick }) {
  ...
  // Visits from more than 14 days ago (but have a lastVisit set)
  const overdue = clients
    ...
    .sort((a, b) => new Date(a.lastVisit) - new Date(b.lastVisit)) // oldest first

  // High chance clients (> 70%)
  const highChance = clients
    .filter(c => Number(c.chance) > 70)
    .sort((a, b) => Number(b.chance) - Number(a.chance))

  // Find the oldest visit date for subtitle
