  ...
  return (
    <>
      <Modal open={open} onClose={onClose} maxWidth={700}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            ...
            {client.address && (
              <div style={{ fontSize: 12, color: 'var(--muted)', marginTop: 5, fontFamily: 'var(--mono)', display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button onClick={openRoute} style={{ padding: '3px 10px', borderRadius: 5, border: '1px solid var(--accent2)', background: 'rgba(92,170,255,0.1)', color: 'var(--accent2)', fontSize: 11, fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'var(--sans)' }}>
                  🧭 Wyznacz trasę
                </button>
              </div>
            )}
        ...
        {/* Info grid */}
        <div className="detail-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ background: 'var(--bg)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, marginBottom: 16 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
            ...
            <button onClick={openGoogleMaps} style={{ padding: '3px 10px', borderRadius: 5, border: '1px solid var(--accent2)', background: 'rgba(92,170,255,0.1)', color: 'var(--accent2)', fontSize: 11, fontWeight: 700, cursor: 'pointer', fontFamily: 'var(--sans)', whiteSpace: 'nowrap' }}>
              🔍 Google Maps
            </button>
          </div>
          {client.hours ? (
            ...
            <div style={{ fontSize: 12, color: 'var(--muted)', fontFamily: 'var(--mono)', fontStyle: 'italic' }}>Brak danych — kliknij "Edytuj" lub "Google Maps"</div>
          )}
        </div>
        ...
        <div className="footer-btns" style={{ display: 'flex', gap: 8, alignItems: 'center', marginTop: 24, borderTop: '1px solid var(--border)', paddingTop: 20, flexWrap: 'wrap' }}>
          <button onClick={() => onDelete(client.id)} style={{ padding: '9px 14px', borderRadius: 8, border: '1px solid var(--danger)', background: 'transparent', color: 'var(--danger)', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 700, cursor: 'pointer', marginRight: 'auto' }}>
            Usuń
          </button>
          <button onClick={() => setScheduleOpen(true)} style={{ padding: '9px 14px', borderRadius: 8, border: '1px solid var(--accent2)', background: 'rgba(92,170,255,0.12)', color: 'var(--accent2)', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            📅 Zaplanuj wizytę
          </button>
          <button onClick={onClose} style={{ padding: '9px 14px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--surface2)', color: 'var(--text)', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            Zamknij
          </button>
          <button onClick={() => onEdit(client)} style={{ padding: '9px 14px', borderRadius: 8, border: 'none', background: 'var(--accent)', color: '#0d0e10', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>
            Edytuj
          </button>
        </div>
      </Modal>
